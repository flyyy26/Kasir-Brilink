...
class AllLogTransaksiScreen extends StatefulWidget {
  const AllLogTransaksiScreen({super.key});

  @override
  State<AllLogTransaksiScreen> createState() => _AllLogTransaksiScreenState();
}

class _AllLogTransaksiScreenState extends State<AllLogTransaksiScreen> with TickerProviderStateMixin {
  ...
  // ============ DAFTAR KATEGORI TRANSAKSI (DIUPDATE) ============
  ...
  // ============ FUNGSI UNTUK MENDAPATKAN SUB_CATEGORY ============
  String _getSubCategory(dynamic account) {
    if (account == null) return '';
    String category = account['category']?.toString().toLowerCase() ?? '';
    String subCategory = account['sub_category']?.toString() ?? '';
    if (category == 'bank' && subCategory.isNotEmpty && subCategory != '-') {
      return ' ($subCategory)';
    }
    return '';
  }

  String _getAccountDisplay(Map<String, dynamic>? account, String defaultName) {
    if (account == null) return defaultName;
    String name = account['name']?.toString() ?? defaultName;
    String subCategory = account['sub_category']?.toString() ?? '';
    ...
    try {
      ...
      if (response.statusCode == 200 && data['status'] == true) {
        ...
        if (outlets.isNotEmpty) {
          ...
          print("Outlet ID: $myOutletId, Pusat ID: $pusatOutletId, Is Pusat: $isPusatOutlet");
        }
      }
    } catch (e) {
  ...
  Future<void> fetchKMCData() async {
    try {
      ...
      if (response.statusCode == 200 && data['status'] == true) {
        ...
        // ============ DEBUG: CETAK DATA DARI API ============
        print("📊 KMC API Response:");
        for (var item in rawData) {
          print("📊 ID: ${item['id']}, Account: ${item['account_name']} (${item['account_sub_category']}), Nominal: ${item['nominal']}");
        }
        ...
        print("📊 KMC List after format: ${kmcList.length} items");
        for (var kmc in kmcList) {
          print("📊 KMC: ${kmc['account_display']} - Rp ${kmc['nominal']}");
        }
      }
    } catch (e) {
  ...
  List<dynamic> getAllData() {
    ...
    for (var kmc in kmcList) {
      // ============ PERBAIKAN: Ambil account_display dari kmcList ============
      String accountDisplay = kmc['account_display'] ?? kmc['account_name'] ?? 'BRI (Penampung Outlet)';
      String accountNameRaw = kmc['account_name_raw'] ?? 'BRI';
      String subCategory = kmc['account_sub_category'] ?? '';
      
      // Format ulang display name
      if (accountDisplay == 'BRI' && subCategory.isNotEmpty && subCategory != '-' && subCategory != 'NULL') {
        accountDisplay = "BRI ($subCategory)";
      }
      
      // ============ DEBUG ============
      print("📊 KMC getAllData: ${kmc['nominal']} - $accountDisplay");
      ...
      allData.add({
        ...
        'sub_category_source': '',
        'sub_category_destination': subCategory,
  ...
  List<dynamic> getFilteredByCategory(String category) {
    ...
    if (category == 'KMC') {
      // Filter transaksi dengan trx_type 'kredit_merchant'
  ...
  // ============ GET COUNT PER OUTLET UNTUK SUB-TAB ============
  ...
  Color _getTrxColor(String type) {
    ...
    if (t.contains('kredit merchant') || t.contains('kmc')) return const Color(0xFF00A86B); // Hijau
  ...
  Widget _buildKMCCard(var item) {
    ...
    // ============ AMBIL AKUN TUJUAN ============
    String accountDisplay = _safeString(item['account_display']);
    if (accountDisplay.isEmpty) {
      accountDisplay = _safeString(item['account_name']);
    }
    if (accountDisplay.isEmpty) {
  ...
  Widget _buildTransactionCard(var trx) {
    ...
    // ============ TENTUKAN DISPLAY TYPE ============
    String displayType = type.toUpperCase();

    // PERBAIKAN: Cek dari yang paling spesifik ke yang paling umum
    if (typeLower.contains('buat piutang')) {
      displayType = "BUAT PIUTANG";
    } else if (typeLower.contains('buat hutang')) {
    ...
    // ============ TAMPILKAN SUB_CATEGORY PADA DESKRIPSI ============
    ...
    // Tambahkan sub_category ke source_display dan destination_display jika ada
  ...
  // ============ BUILD KMC CARD FROM TRANSACTION ============
  Widget _buildKMCCardFromTransaction(var item) {
    double nominal = _safeDouble(item['nominal']) != 0 
        ? _safeDouble(item['nominal']) 
        : _safeDouble(item['nominal_destination']);
    
    String keterangan = _safeString(item['description']);
    String namaKaryawan = _safeString(item['karyawan_name']);
    String createdAt = _safeString(item['created_at']);
    
    // ============ AMBIL DESTINATION ACCOUNT ============
    String destinationDisplay = _safeString(item['destination_display']);
    String destinationAccountName = _safeString(item['destination_account_name']);
    
    // ============ GABUNGKAN UNTUK DISPLAY ============
    String accountDisplay = destinationDisplay.isNotEmpty ? destinationDisplay : destinationAccountName;
    if (accountDisplay.isEmpty) {
      accountDisplay = 'BRI (Penampung Outlet)';
    }
                  ...
                  Container(
                    ...
                    child: TabBar(
                      ...
                      tabs: transactionCategories.map((category) {
                        ...
                        // ============ HAPUS WARNA KHUSUS ============
                        // Semua tab menggunakan warna default
                        return Tab(
                          child: Container(
                            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                            child: Row(
                  ...
                  Expanded(
                    child: TabBarView(
                      ...
                      children: transactionCategories.map((category) {
                        ...
                        Color summaryColor = primaryBlue;
                        if (category == 'Prive') {
                          summaryColor = const Color(0xFF6A1B9A); // Ungu
                        } else if (category == 'Pengeluaran Operasional Konveksi') {
                          summaryColor = const Color(0xFFB71C1C); // Merah
                        } else if (category == 'Pengeluaran Operasional Brilink') {
                          summaryColor = const Color(0xFFE65100); // Orange
                        } else if (category == 'Bayar Piutang') {
                          summaryColor = const Color(0xFF2E7D32);
                        } else if (category == 'Bayar Hutang') {
                          summaryColor = const Color(0xFFD32F2F);
